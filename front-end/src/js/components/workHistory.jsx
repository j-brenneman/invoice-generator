            ...
            {this.customerHistory().map(function (invoice, i) {
              return (
                <tr key={i + invoice.customer._id}>
                  <td>{i +1}</td><td>{invoice.date}</td><td onClick={this.props.selectInvoice.bind(this, this.props.invoices.indexOf(invoice))}><span className="fa fa-file-o view-invoice"></span></td>
                </tr>
              )
            }, this)}
          </tbody>
